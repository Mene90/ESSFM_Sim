function [y1,y2] = ssfm(xx,xy,Px,Tc,alf,bt2,gm,L,Ns)
...
F   = gpuArray(exp(-1i*mod(2*phi,2*pi)));      % transfer function (GVD) of step dz
Fh  = gpuArray(exp(-1i*mod(phi,2*pi)));        % transfer function (GVD) of half step dz/2
...
phi = -geff(1)*(abs(y1).^2 + abs(y2).^2);
y1  = y1.*exp(1i*8/9*phi);
y2  = y2.*exp(1i*8/9*phi);
