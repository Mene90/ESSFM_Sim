function [ data ] = SNR_DISP_COMP_XY(Nstep,NC,dBm,sym_length,n_prop_steps,etasp)
...
n2        = 0;%2.5e-20;              % nonlinear index [m^2/W]
...
%     sig_enh_rx = copy(sig);
    for i = 1:Nspan
        sig_st_rx    = dsp.DBP_vec_ssfm_disp_comp (Pavg(nn)*Loss,sig_st_rx);
...
%     patmaty_enh_rx   = samp2pat(angle(FIELDY_ENH_RX(1:Nt:end)));
    ...
    display(['Power[dBm]   = '   , num2str(dBm(nn))  ,char(9)...
             ...
             'SNR con SSFM = '   , num2str(avg_snr(1))]);
