function [ data ] = BER_FSSFM_XY(Nstep,NC,dBm,sym_length,n_prop_steps,etasp)
...
parfor nn=1:Plen
    ...
    for i = 1:Nspan
     ...
     fmin      = @(C) gpu_vec_fssfm_opt(sig,ux,uy,dsp,C,Nspan,Loss,Hf,bw);
    [Coeff(nn,:),err]=lsqnonlin(fmin,C0,[],[],options);
    
%     fmin       = @(b) gpu_vec_fssfm_opt(sig,ux,uy,dsp,Coeff(nn,:),Nspan,Loss,Hf,b);
%     [BW(nn,:),err]=lsqnonlin(fmin,bw,[],[],options);
end
Coeff.'
...
%             [ux, uy]           = dsp.DBP_gpu_vec_ssfm (Pavg(nn)*Loss,sig_st_rx,ux,uy);
            [ux_enh, uy_enh]   = dsp.DBP_gpu_vec_essfm(Pavg(nn)*Loss,sig_enh_rx,ux_enh,uy_enh);
        end
    else
%         for i = 1:round(Nspan*Nstep)
%             [ux, uy]           = dsp.DBP_gpu_vec_ssfm (Pavg(nn)*Loss,sig_st_rx,ux,uy);
%             [ux_enh, uy_enh]   = dsp.DBP_gpu_vec_filssfm_optimized(Pavg(nn)*Loss,sig_enh_rx,C(nn,:)',ux_enh,uy_enh);
%         end
       [ux_enh, uy_enh]   = dsp.DBP_gpu_vec_filssfm_optimized(Pavg(nn)*Loss,sig_enh_rx,C(nn,:)',ux_enh,uy_enh,Nspan,bw);
    end
    ...
    rotx_st         = angle(mean(FIELDX_ST_RX(1:Nt:end) .*conj(FIELDX_TX)));
    ...
    roty_st         = angle(mean(FIELDY_ST_RX(1:Nt:end) .*conj(FIELDY_TX)));
