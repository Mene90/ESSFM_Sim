...
for j = 1:length(symbols)
    ...
    for i = 1:length(NS)
        ...
        data_2{i} = BER_FSSFM_XY(NS(i),1,dBm , symbols(j), n_prop_steps,etasp);
...
%         BER{i}    = [data_1{i} data_2{i} data_3{i}];
    ...
    legends = { strcat('FESSFM_{XY}   Ns =',{' '}, int2str(NS(1)*40),{' '},'Nc =',{' '}, int2str(Nc(1)-1));...
                strcat('FSSFM_{XY}    Ns =',{' '}, int2str(NS(1)*40));...
%         strcat('Safe_ESSFM_X         Ns =',{' '}, int2str(NS(1)*40),{' '},'Nc =',{' '}, int2str(Nc(1)-1));...
        strcat('ESSFM_{XY}            Ns =',{' '}, int2str(NS(2)*40),{' '},'Nc =',{' '}, int2str(Nc(2)-1));...
        strcat('FSSFM_{XY}            Ns =',{' '}, int2str(NS(2)*40))};...
