function plotfield(pol,flag,col,sig)
...
if Ltime > 0
    time = 0:1/sig.NT:sig.NSYMB-1/sig.NT;
end

isy = ~isempty(sig.FIELDY);  
colt{1} = col;
switch pol
    case 'y'
        nend = double(isy);
...
for k=1:nend    % cycle over the polarizations
    ...
    if Lfreq > 0
        if strcmp(pol,'tot')
            fieldf = sqrt(abs(fft(sig.FIELDX)/Nfft).^2 + ...
                abs(fft(sig.FIELDY)/Nfft).^2);
        else
    ...
    if strcmp(flag(1),'p')  % power in the time domain
        ...
        hold on;
        plot(time,abs(fieldt).^2*1e+3,colt{k});
        xlabel('time [symbols]')
        ylabel('power [mW]')
        grid on
        if k > 1
    ...
    if strcmp(flag(3),'p')  % power in the frequency domain
        ...
        hold on;
        plot(fftshift(sig.FN),fftshift(10*log10(abs(fieldf).^2)),colt{k});
    ...
    elseif strcmp(flag(3),'n')  % normalized power in the frequency domain
        ...
        hold on;
        plot(fftshift(sig.FN),fftshift(10*log10(abs(fieldf).^2/locpow)),colt{k});
        xlabel('frequency [a.u.]')
    ...
    if strcmp(flag(4),'a')  % phase in the frequency domain
        ...
        hold on;
        plot(fftshift(sig.FN),fftshift(angle(fieldf)),colt{k});
